import Link from 'next/link'
import { navItems, siteConfig } from '@/constants'
import { Clock, Phone, MapPin } from 'lucide-react'

export default function Footer() {
  return (
    <footer className="bg-text text-bg border-t border-border">
      <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">

        {/* Core Local SEO Info (NAP) */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">

          <div className="md:col-span-2">
            <Link href="/" className="hover:opacity-80 transition-opacity mb-8 block">
              <img
                src="/footer.png"
                alt="Buzzell Plumbing, Heating & Air"
                className="h-14 w-auto object-contain grayscale brightness-[20] contrast-[200] invert p-1"
              />
            </Link>
            <p className="text-bg/70 text-sm leading-relaxed max-w-sm mb-8">{siteConfig.description}</p>

            <div className="flex items-center gap-6">
              {siteConfig.links.facebook && (
                <a href={siteConfig.links.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook">
                  <svg className="w-5 h-5 text-bg/40 hover:text-bg transition-colors" fill="currentColor" viewBox="0 0 24 24"><path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" /></svg>
                </a>
              )}
              {siteConfig.links.instagram && (
                <a href={siteConfig.links.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram">
                  <svg className="w-5 h-5 text-bg/40 hover:text-bg transition-colors" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" /></svg>
                </a>
              )}
              {siteConfig.links.youtube && (
                <a href={siteConfig.links.youtube} target="_blank" rel="noopener noreferrer" aria-label="YouTube">
                  <svg className="w-5 h-5 text-bg/40 hover:text-bg transition-colors" fill="currentColor" viewBox="0 0 24 24"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z" /></svg>
                </a>
              )}
            </div>
          </div>

          {/* LOCATIONS FOR LOCAL SEO */}
          <div>
            <h3 className="text-sm font-bold text-bg mb-5 uppercase tracking-widest font-mono text-bg/50">Locations</h3>
            <address className="not-italic space-y-4 text-sm text-bg/80">
              <div className="flex items-start gap-2">
                <MapPin className="w-4 h-4 mt-0.5 text-bg/60 shrink-0" />
                <div>
                  <strong>Macon Office</strong><br />
                  1809 Hardeman Ave<br />Macon, GA 31201
                </div>
              </div>
              <div className="flex items-start gap-2">
                <MapPin className="w-4 h-4 mt-0.5 text-bg/60 shrink-0" />
                <div>
                  <strong>Warner Robins Office</strong><br />
                  4811 Russell Pkwy<br />Warner Robins, GA 31088
                </div>
              </div>
            </address>
          </div>

          <div>
            <h3 className="text-sm font-bold text-bg mb-5 uppercase tracking-widest font-mono text-bg/50">Contact 24/7</h3>
            <ul className="space-y-4 text-sm text-bg/80">
              <li className="flex items-start gap-2">
                <Phone className="w-4 h-4 mt-0.5 text-bg/60 shrink-0" />
                <a href="tel:4780000000" className="hover:text-bg transition-colors font-semibold">
                  Call for Emergency
                </a>
              </li>
              <li className="flex items-start gap-2">
                <Clock className="w-4 h-4 mt-0.5 text-bg/60 shrink-0" />
                <span>24/7 Dispatch Available<br /><span className="text-xs opacity-70">No OT charges for VIPs</span></span>
              </li>
              <li>
                <Link href="/contact" className="inline-flex items-center text-sm font-semibold border-b border-bg/30 pb-0.5 hover:border-bg transition-colors">
                  Book Instantly &rarr;
                </Link>
              </li>
            </ul>
          </div>

        </div>

        <div className="pt-8 border-t border-bg/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-bg/60">
          <p>© {new Date().getFullYear()} {siteConfig.name}. All rights reserved.</p>
          <div className="flex gap-6">
            <Link href="/privacy" className="hover:text-bg transition-colors">Privacy Policy</Link>
            <Link href="/terms" className="hover:text-bg transition-colors">Terms of Service</Link>
          </div>
        </div>
      </div>
    </footer>
  )
}
